...
/-!
### Tower form (`IsScalarTower ℚ F L`)

The `IsScalarTower` reformulation of the Kummer core, so the lemmas apply to a relative extension
`ℚ → F → L` directly, without carrying `F`, `L` as intermediate fields of a fixed ambient. The two
compositum bridges (`kw_kummer₀`, `kw_kummer'`) stay in `IntermediateField ℚ L` form and produce the
relative hypotheses assumed here.
-/
...
variable {L : Type*} [Field L] [NumberField L] [Algebra F L] [FiniteDimensional F L]
  [IsGalois F L] [hCF : IsCyclic Gal(L/F)] (hrF : finrank F L = p)
...
Tower form of `kw_kummer₀`: the compositum hypothesis `K ⊔ F = ⊤` is replaced by the degree
`finrank ℚ L = p * (p - 1)` together with `L / ℚ` Galois, so the bridge is pure tower arithmetic and
`K` drops out (the caller supplies the degree from `K`, `F` linearly disjoint).
-/

noncomputable section ScalarTowerBridge

variable (p : ℕ) [hp : Fact p.Prime]
variable {F L : Type*} [Field F] [NumberField F] [Field L] [NumberField L] [Algebra F L]
  [IsCyclotomicExtension {p} ℚ F] [IsGalois ℚ L]
...
lemma kw_kummer₀'₁ (hL : Module.finrank ℚ L = p * (p - 1)) :
  ...
  have hrF : finrank F L = p := by
    have h := Module.finrank_mul_finrank ℚ F L
    rw [hF, hL, mul_comm p (p - 1)] at h
    exact Nat.eq_of_mul_eq_mul_left (by have := hp.out.one_lt; omega) h
  have : IsGalois F L := IsGalois.tower_top_of_isGalois ℚ F L
  exact ⟨this, isCyclic_of_prime_card <| by rw [IsGalois.card_aut_eq_finrank, hrF], hrF⟩
