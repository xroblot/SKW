...
direction is `IsCyclic.subgroup_eq_iff_card_eq`; the converse is
...
theorem isCyclic_iff_subgroup_card_injective {G : Type*} [Group G] [Finite G] :
  ...
  ⟨fun _ _ _ => IsCyclic.subgroup_eq_iff_card_eq.mpr, isCyclic_of_subgroup_card_injective⟩

/-- The quotient of `(ℤ/2^{n+2})ˣ` by the order-2 subgroup `⟨-1⟩` is cyclic (of order `2^n`),
generated by the image of the unit `5`. This is the group-theoretic heart of the cyclicity of
`Gal(ℚ(ζ_{2^{n+2}})⁺/ℚ)` used in the `p = 2` Kronecker-Weber argument.

TODO(SKW): fill the three leaves below. -/
theorem isCyclic_units_two_pow_quotient_neg_one (n : ℕ) :
    IsCyclic ((ZMod (2 ^ (n + 2)))ˣ ⧸ Subgroup.zpowers (-1 : (ZMod (2 ^ (n + 2)))ˣ)) := by
  have hu : IsUnit (5 : ZMod (2 ^ (n + 2))) := by
    -- `5` is odd, hence coprime to `2^{n+2}` (minor numeral-cast friction on
    -- `ZMod.isUnit_iff_coprime`).
    sorry
  -- The image of the unit `5` generates the quotient.
  refine isCyclic_of_orderOf_eq_card (QuotientGroup.mk hu.unit) ?_
  -- Goal: `orderOf ⟦5⟧ = Nat.card (quotient)`. Three sub-facts:
  --  (a) `orderOf hu.unit = 2 ^ n`         -- `ZMod.orderOf_five n` via `orderOf_units`
  --  (b) `-1 ∉ Subgroup.zpowers hu.unit`   -- `5 ≡ 1 [MOD 4]` but `-1 ≡ 3`, so `⟨5⟩ ∩ ⟨-1⟩ = ⊥`,
  --      whence `orderOf ⟦5⟧ = orderOf 5 = 2 ^ n`
  --  (c) `Nat.card (quotient) = 2^{n+1} / 2 = 2 ^ n`  -- `|units| = φ(2^{n+2}) = 2^{n+1}`, `|⟨-1⟩| = 2`
  sorry
