...
theorem ramificationIdx_eq_sub_one₀ [NeZero f] [NeZero m] [Algebra k E] [𝔓.LiesOver 𝔭] [𝔭.LiesOver 𝒑]
  ...
  have htower := Ideal.ramificationIdx_tower (R := ℤ) 𝔭 𝔓
  rw [@IsCyclotomicExtension.Rat.ramificationIdx_eq (m * p) m p 0 hp E _ _ 𝔓 _ _ _
      (by ring) hpm,
...
theorem emultplicity_gaussSum_eq_zero [P.LiesOver 𝒑] {𝔭 : Ideal (𝓞 k)} [NeZero 𝓟]
  ...
  rw [Nat.cast_ne_zero]
  exact (Ideal.ramificationIdx_pos_of_isDedekindDomain' 𝓟 hP₀.ne_zero).ne'
