...
theorem emultiplicity_smul_GaussSum [IsCyclotomicExtension {p} ℚ F]
...
theorem valGauss_add_valGauss_sub_self [𝓟.LiesOver 𝒑] [P.LiesOver 𝒑] {k : ℤ}
    (hk : ↑(p ^ f - 1 : ℕ) ∣ k) (a : ℤ) :
...
theorem exists_valGauss_add_valGauss_eq_valGauss_add_mul [𝓟.LiesOver P] [P.LiesOver 𝒑] (a b : ℤ) :
...
theorem valGauss_eq_self [𝓟.LiesOver P] [P.LiesOver 𝒑] (a : ℕ) (ha : a < p - 1) :
    valGauss hbij hζ 𝓟 a = a := by
  have : 𝓟.LiesOver 𝒑 := LiesOver.trans 𝓟 P 𝒑
...
theorem valGauss_p_sub_one [𝓟.LiesOver P] [P.LiesOver 𝒑] (hf : 2 ≤ f) :
    valGauss hbij hζ 𝓟 (p - 1 : ℕ) = p - 1 := by
  have : 𝓟.LiesOver 𝒑 := LiesOver.trans 𝓟 P 𝒑
...
theorem valGauss_le_sum_digits [𝓟.LiesOver P] [P.LiesOver 𝒑] (a : ℕ) :
...
example [P.LiesOver 𝒑] :
    2 * ∑ a ∈ Finset.range (p ^ f - 1), valGauss hbij hζ 𝓟 a = (p ^ f - 1) * f * (p - 1) := by
  rw [two_mul, ← Fin.sum_univ_eq_sum_range]
