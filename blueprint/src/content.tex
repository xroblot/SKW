...
\begin{proof}
  ...
  \zeta_p^{-1}$. Every unit of the CM field $F$ is a root of unity times a unit of its
  maximal real subfield, so write $\eta = \zeta_p^{t_0} \varepsilon$ with
  $\sigma_{-1}(\varepsilon) = \varepsilon$. With $\mu = \alpha^p \eta$, squaring gives
  \[
    \mu^2 = \alpha^{2p}\, \zeta_p^{t}\, \varepsilon^2, \qquad t := 2t_0 .
  \]
  Since $L/\QQ$ is abelian, \ref{lem:kw_abelian_kummer} applied to $\sigma_{-1}$ gives
  $\xi \in F^\times$ with $\sigma_{-1}(\mu) = \xi^p \mu^{-1}$, hence
  $\sigma_{-1}(\mu^2) = \xi^{2p} \mu^{-2}$. Applying $\sigma_{-1}$ to the displayed identity,
  \[
    \sigma_{-1}(\alpha)^{2p}\, \zeta_p^{-t}\, \varepsilon^2
      = \xi^{2p}\, \alpha^{-2p}\, \zeta_p^{-t}\, \varepsilon^{-2}.
  \]
  Cancelling $\zeta_p^{-t}$ gives
  \[
    \varepsilon^4 = \left(\frac{\xi}{\alpha\, \sigma_{-1}(\alpha)}\right)^{\!2p},
  \]
  so $\varepsilon^4$ is a $p$-th power in $F^\times$. As $p$ is odd, $\gcd(4,p) = 1$, so
  $\varepsilon$ is a $p$-th power, say $\varepsilon = \delta^p$ with $\delta \in \cO_F^\times$.
  Hence $\mu = \alpha^p \zeta_p^{t_0} \varepsilon = (\alpha\delta)^p \zeta_p^{t_0}$ with
  $\alpha\delta \in \cO_F$, and
  \[
    L = F(\sqrt[p]{\mu}) = F(\sqrt[p]{\zeta_p^{t_0}}) \subseteq \QQ(\zeta_{p^2}).
  \]
