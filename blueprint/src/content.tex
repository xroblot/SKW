...
\begin{lemma}
  ...
  Let $L/K$ be a finite Galois extension of number fields, and let $\mathfrak{Q}$ be a prime of
  $L$ above a prime $\mathfrak{p}$ of $K$ whose ramification index $e(\mathfrak{Q}\mid\mathfrak{p})$
  is prime to the residue characteristic of $\mathfrak{p}$ (tame ramification). Then
  \[
    \theta : I(\mathfrak{Q}\mid\mathfrak{p}) \to k(\mathfrak{Q})^\times, \qquad
    \tau \longmapsto \tau(\pi)/\pi \bmod \mathfrak{Q}
  \]
  (for any uniformizer $\pi$ at $\mathfrak{Q}$) is a well-defined injective group homomorphism.
  In particular the inertia group $I(\mathfrak{Q}\mid\mathfrak{p})$ is cyclic of order
  $e(\mathfrak{Q}\mid\mathfrak{p})$.
\end{lemma}

\begin{proof}
  Localize at $\mathfrak{Q}$: write $\mathcal{O} = \mathcal{O}_{L,\mathfrak{Q}}$ for the local
  ring, $\mathfrak{m} = \mathfrak{Q}\mathcal{O}$ its maximal ideal, $v = v_{\mathfrak{Q}}$ the
  valuation, $k(\mathfrak{Q}) = \mathcal{O}/\mathfrak{m}$ the residue field of characteristic
  $\ell$, and $e = e(\mathfrak{Q}\mid\mathfrak{p})$. By definition every $\tau \in I :=
  I(\mathfrak{Q}\mid\mathfrak{p})$ fixes $\mathfrak{Q}$ (hence $v \circ \tau = v$) and acts
  trivially on $k(\mathfrak{Q})$.

  \emph{Well-defined.} For $\tau \in I$ and a uniformizer $\pi$ (i.e.\ $v(\pi) = 1$), one has
  $v(\tau\pi) = v(\pi) = 1$, so $\tau\pi/\pi \in \mathcal{O}^\times$ and its residue
  $\theta(\tau) \in k(\mathfrak{Q})^\times$ is defined. It does not depend on $\pi$: another
  uniformizer is $\pi' = u\pi$ with $u \in \mathcal{O}^\times$, and $\tau\pi'/\pi' =
  (\tau u/u)(\tau\pi/\pi)$; since $\tau$ acts trivially on $k(\mathfrak{Q})$ we have $\tau u
  \equiv u \pmod{\mathfrak{m}}$, so $\tau u/u \equiv 1$ and the residue is unchanged.
  ...
  \[
    \theta(\sigma\tau) = \overline{\sigma\tau\pi/\pi}
      ...
      = \bar{a}\cdot\theta(\sigma) = \theta(\tau)\,\theta(\sigma),
  \]
  where $\overline{\sigma(a)} = \bar a$ because $\sigma$ is trivial on $k(\mathfrak{Q})$. As
  $k(\mathfrak{Q})^\times$ is abelian, $\theta(\sigma\tau) = \theta(\sigma)\theta(\tau)$.

  \emph{Kernel and injectivity.} By construction $\tau \in \ker\theta$ iff $\tau\pi \equiv \pi
  \pmod{\mathfrak{m}^2}$, i.e.\ $\tau$ lies in the first ramification group (in lower numbering)
  $G_1 = \{\tau \in I : v(\tau x - x) \geq 2 \text{ for all } x \in \mathcal{O}\}$. Hence $\theta$
  factors through an injection $I/G_1 \hookrightarrow k(\mathfrak{Q})^\times$. Each quotient
  $G_i/G_{i+1}$ ($i \geq 1$) embeds into the additive group of $k(\mathfrak{Q})$, so $G_1$ is an
  $\ell$-group. Tameness means $\ell \nmid e = |I|$; since $G_1 \leq I$ is an $\ell$-group, this
  forces $G_1 = 1$, and $\theta$ is injective.

  \emph{Cyclicity.} The image $\theta(I)$ is a finite subgroup of the multiplicative group of the
  field $k(\mathfrak{Q})$, hence cyclic. Therefore $I \cong \theta(I)$ is cyclic of order $|I| =
  e$, with image $\theta(I) = \mu_e\big(k(\mathfrak{Q})\big)$. \emph{(Cf.\ Serre, \emph{Local
  Fields}, Ch.~IV §1--2.)}
\end{proof}
...
\begin{proof}
  Let $\tau$ generate $I = I(\mathfrak{Q}\mid\mathfrak{p})$ and let $\sigma$ in the decomposition
  group lift the Frobenius $x \mapsto x^{N\mathfrak{p}}$ of $k(\mathfrak{Q})/k(\mathfrak{p})$.
  The tame character of \ref{lem:tame_inertia_cyclic} satisfies $\theta(\sigma\tau\sigma^{-1}) =
  \theta(\tau)^{N\mathfrak{p}}$, i.e.\ $\sigma\tau\sigma^{-1} = \tau^{N\mathfrak{p}}$. Since
  $L/K$ is abelian, $\sigma\tau\sigma^{-1} = \tau$, whence $\tau^{N\mathfrak{p}-1} = 1$ and
  $e = \operatorname{ord}(\tau)$ divides $N\mathfrak{p} - 1$.
\end{proof}
