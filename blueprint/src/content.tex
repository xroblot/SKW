...
\begin{lemma}
  ...
  \uses{def:teichmuller, lem:teichmuller_order}
  \leanok
...
\begin{proof}
  ...
  By \ref{lem:teichmuller_frob}, $\sigma(\omega(x)) = \omega(x)^{\fn(\sigma)}$
  for all $x$. Therefore
  \[
...
\begin{lemma}
  \label{lem:s_sum}
  \uses{def:s, lem:s_sum_dvd, lem:s_eq_zero}
  \leanok
  We have
  \[
...
\begin{lemma}
  ...
  \uses{lem:gauss_sum_in_cyclotomic, lem:L_E_unramified,
        ...
        lem:galois_gauss_sum_a, lem:gauss_sum_galois_action_K, def:s,
        lem:s_fractional_part}
  For $a \in (\ZZ/m\ZZ)^\times$,
  \[
    v_{\sigma_a^{-1}(\tilde{\fp}_0)}(g_d^m) = m \sum_{i=0}^{f-1}
    \left\{ \frac{ap^i}{m} \right\}.
  \]
\end{lemma}
...
\begin{proof}
  ...
  \pmod{m}$. By \ref{lem:galois_gauss_sum_a},
  ...
  Therefore $v_{\sigma_a^{-1}(\tilde{\fp}_0)}(g_d^m) = \frac{m \cdot s(ad)}{p-1}
  = m \sum_{i=0}^{f-1} \left\{ \frac{ap^i}{m} \right\}$,
  where the last equality is \ref{lem:s_fractional_part}.
\end{proof}
...
\begin{lemma}
  ...
  For $a \in (\ZZ/m\ZZ)^\times$,
  \[
    v_{\sigma_a^{-1}(\tilde{\fp}_0)}(\tilde{\fp}_0^{m\theta}) =
    m \sum_{i=0}^{f-1} \left\{ \frac{ap^i}{m} \right\}.
  \]
\end{lemma}

\begin{proof}
  By definition of $\theta$ and \ref{lem:stabilizer_tP0}, regrouping the
  sum over $(\ZZ/m\ZZ)^\times$ by orbits under $\langle p \rangle$:
  \[
    v_{\sigma_a^{-1}(\tilde{\fp}_0)}(\tilde{\fp}_0^{m\theta})
    = m \sum_{b \in (\ZZ/m\ZZ)^\times} \left\{\frac{b}{m}\right\}
      v_{\sigma_a^{-1}(\tilde{\fp}_0)}(\sigma_b^{-1}(\tilde{\fp}_0))
    = m \sum_{i=0}^{f-1} \left\{ \frac{ap^i}{m} \right\},
  \]
  since $v_{\sigma_a^{-1}(\tilde{\fp}_0)}(\sigma_b^{-1}(\tilde{\fp}_0)) = 1$
  if $b \in a\langle p \rangle$ and $0$ otherwise.
\end{proof}

\begin{lemma}
  \label{lem:gauss_sum_factorization_cyclotomic}
  \uses{lem:gauss_sum_power_in_cyclotomic, lem:gauss_sum_norm,
        lem:valuation_gauss_sum_power, lem:valuation_stickelberger,
        lem:stabilizer_tP0}
  \[
    (g_d^m) = \tilde{\fp}_0^{m\theta}
  \]
...
\begin{proof}
  ...
  By \ref{lem:gauss_sum_norm}, $g_d^m$ is only divisible by primes above $p$.
  Similarly, $\tilde{\fp}_0^{m\theta}$ only involves primes above $p$.
  It therefore suffices to check that for every prime $\mathfrak{q} =
  \sigma_a^{-1}(\tilde{\fp}_0)$ of $\cO_{\QQ(\zeta_m)}$ above $p$,
  \[
    v_{\mathfrak{q}}(g_d^m) = v_{\mathfrak{q}}(\tilde{\fp}_0^{m\theta}).
  \]
  \[
    v_{\mathfrak{q}}(g_d^m) = m \sum_{i=0}^{f-1} \left\{ \frac{ap^i}{m} \right\}
  \]
  by \ref{lem:valuation_gauss_sum_power}. On the other hand, by
  \ref{lem:valuation_stickelberger},
  \[
    v_{\mathfrak{q}}(\tilde{\fp}_0^{m\theta}) = m \sum_{i=0}^{f-1}
    \left\{ \frac{ap^i}{m} \right\}.
  \]
...
\begin{theorem}
  ...
  The ideal $\tilde{\fp}_0^{m\theta}$ is principal in $\QQ(\zeta_m)$.
\end{theorem}
...
\begin{proof}
  ...
  $\mathfrak{a}^{p\theta}$ is principal in $\QQ(\zeta_p)$. Since
  ...
  $[\mathfrak{a}]^{p\theta} = 1$ (as $\mathfrak{a}^{p\theta}$ is principal),
