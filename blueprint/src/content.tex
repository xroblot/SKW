...
\begin{proof}
  ...
  \zeta_p^{-1}$. Every unit of the CM field $F$ is a root of unity times a unit of
  its maximal real subfield, so we may write $\eta = \zeta_p^t \varepsilon$ with
  $\sigma_{-1}(\varepsilon) = \varepsilon$. Since $L/\QQ$ is abelian,
  \ref{lem:kw_abelian_kummer} applied to $\sigma_{-1}$ gives $\xi \in F^\times$ with
  $\sigma_{-1}(\mu) = \xi^p \mu^{-1}$. From $\mu = \alpha^p \zeta_p^t \varepsilon$,
  \[
    \sigma_{-1}(\mu) = \sigma_{-1}(\alpha)^p\, \zeta_p^{-t}\, \varepsilon
    \quad \text{and} \quad
    \xi^p \mu^{-1} = \xi^p \alpha^{-p}\, \zeta_p^{-t}\, \varepsilon^{-1}.
  \]
  Equating and cancelling $\zeta_p^{-t}$ gives $\sigma_{-1}(\alpha)^p \varepsilon =
  \xi^p \alpha^{-p} \varepsilon^{-1}$, that is,
  \[
    \varepsilon^2 = \left(\frac{\xi}{\alpha\, \sigma_{-1}(\alpha)}\right)^{\!p},
  \]
  so $\varepsilon^2$ is a $p$-th power in $F^\times$. As $p$ is odd, $\gcd(2,p) = 1$,
  and writing $1 = 2u + pv$ we get $\varepsilon = (\varepsilon^2)^u (\varepsilon^v)^p$,
  a $p$-th power; say $\varepsilon = \delta^p$ with $\delta \in \cO_F^\times$ (its
  $p$-th root is again a unit). Hence $\mu = (\alpha\delta)^p \zeta_p^t$ with
  $\alpha\delta \in \cO_F$, and
  \[
