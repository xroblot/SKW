...
\begin{proof}
  Let $\sigma_{-1}$ denote complex conjugation on $F$, so $\sigma_{-1}(\zeta_p) =
  \zeta_p^{-1}$. For every unit $\eta \in \cO_F^\times$, the ratio $\eta/\sigma_{-1}(\eta)$
  has absolute value $1$ at every archimedean place, hence is a root of unity, while
  $\eta\,\sigma_{-1}(\eta)$ is a real unit. Their product being $\eta^2$, the \emph{square}
  of every unit is a root of unity times a real unit (no assumption on the unit index is
  needed, since we only use the square):
  \[
    \eta^2 = \zeta_p^{t}\, \varepsilon, \qquad
    \varepsilon \in \cO_F^\times,\ \sigma_{-1}(\varepsilon) = \varepsilon .
  \]
  With $\mu = \alpha^p \eta$ this gives $\mu^2 = \alpha^{2p}\, \zeta_p^{t}\, \varepsilon$.
  Since $L/\QQ$ is abelian, \ref{lem:kw_abelian_kummer} applied to $\sigma_{-1}$ gives
  $\xi \in F^\times$ with $\sigma_{-1}(\mu) = \xi^p \mu^{-1}$, hence
  $\sigma_{-1}(\mu^2) = \xi^{2p} \mu^{-2}$. Applying $\sigma_{-1}$ to $\mu^2 = \alpha^{2p}
  \zeta_p^{t} \varepsilon$,
  \[
    \sigma_{-1}(\alpha)^{2p}\, \zeta_p^{-t}\, \varepsilon
      = \xi^{2p}\, \alpha^{-2p}\, \zeta_p^{-t}\, \varepsilon^{-1}.
  \]
  Cancelling $\zeta_p^{-t}$ gives
  $\varepsilon^2 = \bigl(\xi/(\alpha\, \sigma_{-1}(\alpha))\bigr)^{2p}$, so $\varepsilon^2$
  is a $p$-th power in $F^\times$. As $p$ is odd, $\gcd(2,p) = 1$, so $\varepsilon$ is a
  $p$-th power, say $\varepsilon = \delta^p$. Hence
  \[
    \mu^2 = \alpha^{2p}\, \zeta_p^{t}\, \delta^p = \zeta_p^{t}\, (\alpha^2 \delta)^p,
  \]
  so $\mu^2 \in \zeta_p^{\ZZ} (F^\times)^p$. Since $p$ is odd, squaring is invertible on
  $F^\times/(F^\times)^p$, so already $\mu \in \zeta_p^{\ZZ}(F^\times)^p$; write
  $\mu = \zeta_p^{t'} \beta^p$ with $\beta \in \cO_F$. Therefore
  \[
    L = F(\sqrt[p]{\mu}) = F(\sqrt[p]{\zeta_p^{t'}}) \subseteq \QQ(\zeta_{p^2}),
  \]
  and since $[L:F] = p = [\QQ(\zeta_{p^2}):F]$, we conclude $L = \QQ(\zeta_{p^2})$.
\end{proof}
