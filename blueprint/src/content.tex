...
\begin{proof}
  \leanok
  Let $\sigma_{-1}$ denote complex conjugation on $F = \QQ(\zeta_p)$, so
  $\sigma_{-1}(\zeta_p) = \zeta_p^{-1}$. For every unit $\eta \in \cO_F^\times$, the ratio
  $\eta/\sigma_{-1}(\eta)$ has absolute value $1$ at every archimedean place, hence is a root
  of unity, while $\eta\,\sigma_{-1}(\eta)$ is a real unit; their product being $\eta^2$, the
  square of every unit factors as a root of unity times a real unit,
  $\eta^2 = \zeta\,\varepsilon_0$ with $\sigma_{-1}(\varepsilon_0) = \varepsilon_0$. Squaring
  once more,
  \[
    \eta^4 = \zeta^2\, \varepsilon, \qquad \varepsilon = \varepsilon_0^2,\
    \sigma_{-1}(\varepsilon) = \varepsilon .
  \]
  The roots of unity in $F$ form $\mu_{2p}$, so $\zeta^{2p} = 1$ and hence
  $(\zeta^2)^p = \zeta^{2p} = 1$: the factor $\zeta^2$ is a genuine $p$-th root of unity.
  (Working with $\eta^4$ rather than $\eta^2$ is exactly what lands the torsion factor in
  $\mu_p$, sidestepping any sign normalization.)

  Since $L/\QQ$ is abelian, \ref{lem:kw_abelian_kummer} applied to $\sigma_{-1}$, combined with
  $\mu = \alpha^p \eta$, yields $w \in F^\times$ with $\eta\,\sigma_{-1}(\eta) = w^p$
  (explicitly $w = \xi/(\alpha\,\sigma_{-1}(\alpha))$, where $\sigma_{-1}(\mu) = \xi^p
  \mu^{-1}$). Applying $\sigma_{-1}$ to $\eta^4 = \zeta^2 \varepsilon$ gives
  $\sigma_{-1}(\eta)^4 = \zeta^{-2} \varepsilon$, and multiplying the two relations the factor
  $\zeta^2$ cancels:
  \[
    \varepsilon^2 = \bigl(\eta\,\sigma_{-1}(\eta)\bigr)^4 = w^{4p} = (w^4)^p .
  \]
  As $p$ is odd, $\gcd(2,p) = 1$, so $\varepsilon$ is itself a $p$-th power, $\varepsilon =
  \delta^p$. Hence
  \[
    \eta^4 = \zeta^2\, \delta^p, \qquad
    \mu^4 = \alpha^{4p}\, \zeta^2\, \delta^p = \zeta^2\,(\alpha^4 \delta)^p \in
    \zeta_p^{\ZZ} (F^\times)^p .
  \]
  Finally, as $\gcd(4,p) = 1$ we have $F(\sqrt[p]{\mu}) = F(\sqrt[p]{\mu^4})$, and
  $\mu^4 \in \zeta_p^{\ZZ}(F^\times)^p$ gives $F(\sqrt[p]{\mu^4}) = F(\sqrt[p]{\zeta^2})
  \subseteq \QQ(\zeta_{p^2})$. Since $[L:F] = p = [\QQ(\zeta_{p^2}):F]$, we conclude
  $L = \QQ(\zeta_{p^2})$.
\end{proof}
