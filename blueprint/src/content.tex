...
\begin{definition}
  ...
  \[
    g_a = \begin{cases}
      2 & \text{if } q = 2, \\
      g(\om^{-a}, \psi) = \displaystyle\sum_{x \in \cO_K/P} \om(x)^{-a} \cdot \psi(x) & \text{otherwise.}
    \end{cases}
  \]
\end{definition}
...
\begin{lemma}
  ...
  Assume $q \neq 2$. Then for all $a \in \ZZ$, $s(a) = v_{\fP}(g_a)$.
\end{lemma}
...
\begin{lemma}
  ...
  \uses{lem:val_galois_gauss_sum, lem:primes_above_p, lem:gauss_sum_ne_zero, lem:s_frob}
  For all $a \in \ZZ$ with $(q-1) \nmid a$,
  \[
...
\begin{proof}
  Since $(q-1) \nmid a$, we have $g_a \neq 0$ by \ref{lem:gauss_sum_ne_zero}.
  The only primes dividing $(g_a)$ are those above $p$, since $g_a$ is an
  algebraic integer whose norm is a power of $p$. By \ref{lem:primes_above_p},
