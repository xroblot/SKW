...
\begin{proof}
  We have $\omega^{-a} = 1$, so $g_a = g_0 = -1$
...
\begin{lemma}
  ...
  \lean{multiplicity_smul_GaussSum}
  \leanok
  For all $\sigma \in \Gal(L/F)$,
  \[
...
\begin{lemma}
  \label{lem:s_sum_dvd}
  \uses{def:s, def:gauss_sum_s, lem:teichmuller_order, lem:ramification_idx, lem:s_eq_zero}
...
\begin{lemma}
  \label{lem:s_congr}
  \uses{def:s, lem:gauss_sum_mul, lem:val_jac_pow, lem:s_sum_dvd, lem:s_eq_zero}
  For all $a, b \in \ZZ$,
  \[
...
\begin{proof}
  ...
  If $(q-1) \mid (a+b)$, then $s(a+b) = 0$ by \ref{lem:s_eq_zero}, so we must show $s(a) + s(b) \equiv 0
  \pmod{p-1}$. Apply \ref{lem:s_sum_dvd} with $k = a+b$ (which satisfies $(q-1) \mid k$):
  if $(q-1) \mid a$ then $(q-1) \mid b$ as well, so $s(a) = s(b) = 0$ by \ref{lem:s_eq_zero};
