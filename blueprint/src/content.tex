...
\begin{lemma}
  ...
  \uses{def:gauss_sum_s, lem:addchar_primitive, lem:teichmuller_order, lem:gauss_sum_mem}
  \lean{norm_GaussSum}
  \leanok
...
\begin{proof}
  \leanok
  Since $(q-1) \nmid a$, $\omega^{-a}$ is nontrivial by
  \ref{lem:teichmuller_order}, and $\psi$ is primitive by
  \ref{lem:addchar_primitive}. By \texttt{gaussSum\_mul\_gaussSum\_eq\_card},
  \[
    g_a \cdot g_{-a} = \omega^a(-1) \cdot q = \pm p^f.
  \]
...
\begin{proof}
  ...
  If $(q-1) \nmid a$, then $\omega^{-a}$ is a nontrivial character and $\psi$
  is primitive by \ref{lem:addchar_primitive}. By
  \texttt{gaussSum\_mul\_gaussSum\_eq\_card},
  $g(\omega^{-a}, \psi) \cdot g(\omega^{a}, \psi^{-1}) = q = p^f$.
  Now $g(\omega^{a}, \psi^{-1}) = \omega^{a}(-1) \cdot g_{-a}$ (by a change
  of variable $x \mapsto -x$), so $g_a \cdot g_{-a} = \omega^{a}(-1) \cdot p^f$.
...
\begin{proof}
  ...
  If $(q-1) \mid (a+b)$, then $s(a+b) = 0$, so we must show $s(a) + s(b) \equiv 0
  \pmod{p-1}$. Apply \ref{lem:s_sum_dvd} with $k = a+b$ (which satisfies $(q-1) \mid k$):
  if $(q-1) \mid a$ then $(q-1) \mid b$ as well, so $s(a) = s(b) = 0$;
  otherwise $s(a) + s(b) = (p-1) \cdot f \equiv 0 \pmod{p-1}$.
  In both cases $s(a) + s(b) \equiv 0 = s(a+b) \pmod{p-1}$.
\end{proof}
...
\begin{proof}
  ...
  By \ref{lem:addchar_frob}, $\psi(\varphi^{-1}(y)) = \psi(y)$. Therefore $g_{pa} = g_a$,
  and $s(pa) = v_{\fP}(g_{pa}) = v_{\fP}(g_a) = s(a)$.
\end{proof}
...
\begin{proof}
  ...
  if $a_0 = p-1$, since $a < q-1$, we have $a' \geq 1$. If $f = 1$ then
  $q = p$ and $a < p-1$, so $a_0 \leq p-2$, contradicting $a_0 = p-1$.
  Hence $f \geq 2$, and \ref{lem:s_p_minus_one} gives $s(p-1) = p-1 \leq p-1$.
...
\begin{proof}
  ...
  Note that $q - 2 = 2(2^{f-1} - 1)$ is even, so $(q-2)/2$ is an integer.
  The set $\{1, \ldots, q-2\}$ consists of $(q-2)/2$ pairs $\{a, q-1-a\}$,
  each contributing $(p-1)f = f$. Therefore
  \[
    \sum_{a=0}^{q-2} s(a) = \frac{q-2}{2} \cdot f = \frac{(q-2) \cdot f \cdot (p-1)}{2}.
  \]
\end{proof}
...
\begin{proof}
  ...
  \ref{lem:val_galois_gauss_sum}. By \ref{lem:primes_above_p}, $\fQ$ appears
...
The \emph{Stickelberger ideal} $I(\QQ(\zeta_m))$ is defined as
$\ZZ[G] \cap \theta \ZZ[G]$, i.e., those $\ZZ[G]$-multiples of $\theta$
...
\begin{lemma}
  \label{lem:stickelberger_prime_power}
  \uses{lem:gauss_sum_factorization_cyclotomic}
  Let $\mathfrak{p}_0$ be a prime of $\QQ(\zeta_m)$ with $\mathfrak{p}_0 \nmid m$,
  let $\beta \in \ZZ[G]$, and suppose $\beta\theta \in \ZZ[G]$. Then
  $\mathfrak{p}_0^{\beta\theta}$ is principal in $\QQ(\zeta_m)$.
\end{lemma}

\begin{proof}
  By \ref{lem:gauss_sum_factorization_cyclotomic},
  $(g(\chi_{\mathfrak{p}_0})^m) = \mathfrak{p}_0^{m\theta}$. Therefore
  \[
    \mathfrak{p}_0^{m\beta\theta} = \left(g(\chi_{\mathfrak{p}_0})^m\right)^\beta
    = \left(g(\chi_{\mathfrak{p}_0})^{m\beta}\right)
  \]
  is principal in $\QQ(\zeta_m, \zeta_p)$.
  Since $g(\chi_{\mathfrak{p}_0})^{m\beta} \in \QQ(\zeta_m)$ by
  \ref{lem:gauss_sum_power_in_cyclotomic}, it is principal in $\QQ(\zeta_m)$.
  Since the monoid of fractional ideals is torsion-free, $\mathfrak{p}_0^{\beta\theta}$
  is principal.
\end{proof}

\begin{lemma}
  \label{lem:stickelberger_coprime_m}
  \uses{lem:stickelberger_prime_power}
  Let $A$ be a fractional ideal of $\QQ(\zeta_m)$ with $(A, m) = 1$,
  let $\beta \in \ZZ[G]$, and suppose $\beta\theta \in \ZZ[G]$. Then
  $A^{\beta\theta}$ is principal.
\end{lemma}

\begin{proof}
  Write $A = \prod_i \mathfrak{p}_i$ with each $\mathfrak{p}_i \nmid m$.
  By \ref{lem:stickelberger_prime_power}, each $\mathfrak{p}_i^{\beta\theta}$
  is principal, hence so is $A^{\beta\theta} = \prod_i \mathfrak{p}_i^{\beta\theta}$.
\end{proof}

\begin{theorem}
  \label{thm:stickelberger}
  \uses{lem:stickelberger_coprime_m}
  Let $A$ be a fractional ideal of $\QQ(\zeta_m)$, let $\beta \in \ZZ[G]$,
  and suppose $\beta\theta \in \ZZ[G]$. Then $A^{\beta\theta}$ is principal.
  Therefore, the Stickelberger ideal annihilates the ideal class group of
  $\QQ(\zeta_m)$.
\end{theorem}

\begin{proof}
  Write $A = (a) A_1$ with $a \in \QQ(\zeta_m)^\times$ and $(A_1, m) = 1$.
  Then $A^{\beta\theta} = (a^{\beta\theta}) A_1^{\beta\theta}$.
  The factor $(a^{\beta\theta})$ is principal by definition. The factor
  $A_1^{\beta\theta}$ is principal by \ref{lem:stickelberger_coprime_m}.
  Therefore $A^{\beta\theta}$ is principal.
\end{proof}
...
\begin{proposition}
  ...
  \uses{prop:kw_exponent_p, lem:kw_cyclic_compositum}
...
\begin{proof}
  ...
  Now apply \ref{thm:stickelberger} with $M = F$, $m = p$, and
  $\theta = \sum_{a=1}^{p-1} \{a/p\} \sigma_a^{-1} = \frac{1}{p}\sum_{a=1}^{p-1}
  a\sigma_a^{-1}$. Taking $\beta = p \in \ZZ[G]$, we have $\beta\theta =
  \sum_{a=1}^{p-1} a\sigma_a^{-1} \in \ZZ[G]$. By \ref{thm:stickelberger},
  $[\mathfrak{a}]^{\beta\theta} = 1$. We compute
  \[
    [\mathfrak{a}]^{\beta\theta} = \prod_{a=1}^{p-1} \sigma_a^{-1}([\mathfrak{a}])^a
...
\begin{proof}
  Since $\chi \neq 1$ and $\psi$ is primitive, we have
  $g(\chi, \psi) \cdot g(\chi^{-1}, \psi^{-1}) = q \neq 0$
  (see \texttt{gaussSum\_mul\_gaussSum\_eq\_card} in Mathlib),
  hence $g(\chi, \psi) \neq 0$.
\end{proof}
...
\begin{proof}
  Since $\psi$ takes values in $\mu_p(\cO_L)$ and $\psi$ is nontrivial by
  \ref{lem:addchar_ne_one}, we have $\psi \neq 1$. Therefore
